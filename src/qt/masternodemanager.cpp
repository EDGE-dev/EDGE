...
static void NotifyedgeNodeUpdated(MasternodeManager *page, CedgeNodeConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updateedgeNode", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifyedgeNodeChanged.connect(boost::bind(&NotifyedgeNodeUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifyedgeNodeChanged.disconnect(boost::bind(&NotifyedgeNodeUpdated, this, _1));
}
...
void MasternodeManager::updateedgeNode(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_edge);
    bool bFound = false;
    int nodeRow = 0;
...
void MasternodeManager::updateNodeList()
{
    ...
    {
        LOCK(cs_edge);
        BOOST_FOREACH(PAIRTYPE(std::string, CedgeNodeConfig) edge, pwalletMain->mapMyedgeNodes)
        {
            updateedgeNode(QString::fromStdString(edge.second.sAlias), QString::fromStdString(edge.second.sAddress), QString::fromStdString(edge.second.sMasternodePrivKey), QString::fromStdString(edge.second.sCollateralAddress));
        }
    }
}
...
{
    AddEditedgeNode* aenode = new AddEditedgeNode();
    aenode->exec();
}
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CedgeNodeConfig c = pwalletMain->mapMyedgeNodes[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    edgeNodeConfigDialog* d = new edgeNodeConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    confirm = QMessageBox::question(this, "Delete edge Node?", "Are you sure you want to delete this edge node configuration?", QMessageBox::Yes|QMessageBox::No);
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMyedgeNodes.erase(sAddress);
        walletdb.EraseedgeNodeConfig(c.sAddress);
...
void MasternodeManager::on_startButton_clicked()
{
    ...
    QMessageBox msg;
    if(result)
        msg.setText("edge Node at " + QString::fromStdString(c.sAddress) + " started.");
    else
...
void MasternodeManager::on_stopButton_clicked()
{
    ...
    if(result)
    {
        msg.setText("edge Node at " + QString::fromStdString(c.sAddress) + " stopped.");
    }
    else
    {
        msg.setText("Error: " + QString::fromStdString(errorMessage));
