...
bool CWalletDB::WriteedgeNodeConfig(std::string sAlias, const CedgeNodeConfig& nodeConfig)
{
    nWalletDBUpdated++;
    return Write(std::make_pair(std::string("edge"), sAlias), nodeConfig, true);
}

bool CWalletDB::ReadedgeNodeConfig(std::string sAlias, CedgeNodeConfig& nodeConfig)
{
    return Read(std::make_pair(std::string("edge"), sAlias), nodeConfig);
}

bool CWalletDB::EraseedgeNodeConfig(std::string sAlias)
{
    nWalletDBUpdated++;
    return Erase(std::make_pair(std::string("edge"), sAlias));
}
             ...
             CWalletScanState &wss, string& strType, string& strErr)
{
    try {
        ...
        else if (strType == "edge")
	{
	    std::string sAlias;
	    ssKey >> sAlias;
	    CedgeNodeConfig edgeNodeConfig;
	    ssValue >> edgeNodeConfig;
	    pwallet->mapMyedgeNodes.insert(make_pair(sAlias, edgeNodeConfig));
	}
    } catch (...)
    {
        return false;
