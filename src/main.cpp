...
const string strMessageMagic = "edge Signed Message:\n";
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    if (pindex->pprev)
    {
        CDiEDGElockIndex blockindexPrev(pindex->pprev);
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (!txdb.WriteBlockIndex(CDiEDGElockIndex(pindex)))
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    if (!txdb.TxnBegin())
        return false;
    txdb.WriteBlockIndex(CDiEDGElockIndex(pindexNew));
    if (!txdb.TxnCommit())
        return false;
