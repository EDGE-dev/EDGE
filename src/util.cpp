...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\edge
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\edge
    // Mac: ~/Library/Application Support/edge
    // Unix: ~/.edge
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "edge";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "edge.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "edged.pid"));
