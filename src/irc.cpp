...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #edgecoinTEST\r");
            Send(hSocket, "WHO #edgecoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #edgecoin\n");
            // randomly join #edgecoin00-#edgecoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #edgecoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #edgecoin%02d\r", channel_number).c_str());
        };
