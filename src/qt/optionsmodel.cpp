...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeedgeAmount"))
        settings.setValue("nAnonymizeedgeAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeedgeAmount = settings.value("nAnonymizeedgeAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeedgeAmount"))
        SoftSetArg("-anonymizeedgeamount", settings.value("nAnonymizeedgeAmount").toString().toStdString());
#ifdef USE_NATIVE_I2P
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeedgeAmount:
            nAnonymizeedgeAmount = value.toInt();
            settings.setValue("nAnonymizeedgeAmount", nAnonymizeedgeAmount);
            emit anonymizeedgeAmountChanged(nAnonymizeedgeAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
